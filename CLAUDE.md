...
## Installed skills (`.claude/skills/`, gitignored)

The ui-ux-pro-max skill pack is installed as local tooling. It ships several skills that would
conflict with this repo's locked brand system if used naively, so the scope is explicit:

- **`brand/tokens.json` is the single source of truth. `tokens.css` is generated by
  `npm run tokens`. Neither is ever written by an installed skill.**
- **Never run `sync-brand-to-tokens.cjs` or any other installed skill script against this repo's
  `brand/` directory.**
- **Of the installed pack, only `ui-ux-pro-max` is in scope, and only for interaction and UX
  guidance** — form patterns, disclosure, accessibility, motion. Use `--domain ux` and
  `--stack astro`.
- **Do not run `--design-system`.** It generates a full palette and typography set and would
  conflict with the approved brand.
- **The `brand`, `design-system`, `ui-styling`, `banner-design`, and `slides` skills are out of
